...
export const ExperienceDetails: React.FC<IProps> = ({
  formValues,
  setFieldValue,
  onNext,
  onBack,
  ...
  const handleAdd = async () => {
    try {
      await ExperienceSchema.validate(formValues, { abortEarly: false });
      const val = {
        ...formValues?.experience,
        kind: sectionType,
        fromDate: formValues?.experience?.duration?.fromDate,
        toDate: formValues?.experience?.duration?.toDate
      };
      delete val?.duration
  ...
  const handleDateChange = (date: { fromDate: number | null; toDate: number | null }) => {
    ...
    setFieldValue('experience.duration', { fromDate, toDate });
  };
      ...
      {showForm ? (
        <div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <ApTextInput label="Role" name="experience.role" placeHolder="Enter Role" />

            <ApDateRangePicker
              fromLabel="Start Date"
              toLabel="End Date"
              date={formValues?.experience?.duration}
                ...
                .map((item: any, index: number) => (
                  ...
                  >
                    <div className="flex flex-col w-full gap-3">
                      ...
                      <p className="font-bold">
                        Role
                        <span className="block text-sm font-light">{item.role}</span>
                      </p>
                      <p className="font-bold">
