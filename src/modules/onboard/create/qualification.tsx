...
import { QualificationSchema } from '../validation';
...
export const QualificationDetails: React.FC<IProps> = ({
  formValues,
  setFieldValue,
  onNext,
  onBack,
  ...
  const handleAdd = async () => {
    try {
      await QualificationSchema.validate(formValues, { abortEarly: false });
      const val = {
        ...formValues?.qualification,
        kind: sectionType,
        fromDate: formValues?.qualification?.duration?.fromDate,
        toDate: formValues?.qualification?.duration?.toDate
      };
      delete val?.duration
  ...
  const handleDateChange = (date: { fromDate: number | null; toDate: number | null }) => {
    ...
    setFieldValue('qualification.duration', { fromDate, toDate });
  };
        ...
        <div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <ApTextInput
              ...
              name="qualification.fieldOfStudy"
            ...
            <ApDateRangePicker
              ...
              toLabel="End Date"
              date={formValues?.qualification?.duration}
