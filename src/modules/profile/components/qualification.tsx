...
export default function QualificationInformation({ profile, onEdit }: IProps) {
  ...
  return (
    <div className="border rounded-lg p-4 mb-4 shadow-md">
      ...
      {profile?.sections
        ?.filter((item) => item.kind == 'QUALIFICATION')
        ?.map((item, i) => (
          <div
            key={i}
            className={`flex flex-col gap-3 mt-3 ${
              profile?.sections?.filter((item) => item.kind == 'QUALIFICATION')?.length !== i + 1 &&
              'border-b pb-3'
            }`}
          >
