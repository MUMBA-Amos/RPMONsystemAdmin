...
export default function AwardInformation({ profile, onEdit }: IProps) {
  ...
  return (
    <div className="border rounded-lg p-4 mb-4 shadow-md">
      ...
      {profile?.sections
        ?.filter((item) => item.kind == 'AWARDS')
        ?.map((item, i) => (
          <div
            key={i}
            className={`flex flex-col gap-3 mt-3 ${
              profile?.sections?.filter((item) => item.kind == 'AWARDS')?.length !== i + 1 &&
              'border-b pb-3'
            }`}
          >
            <div className="flex justify-between">
              <p className="text-primary font-medium">Awarding Body</p>
              <p className="text-sm">{item?.institution}</p>
            </div>

            <div className="flex justify-between">
              <p className="text-primary font-medium">Award Name:</p>
